    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-xl sm:rounded-lg p-6">
                ...
                <form action="{{ route('filiere.update', ['code_etape' => $filiere->code_etape, 'id_session' => $filiere->id_session]) }}" method="POST">
                    @csrf
                    @method('PUT')
                            ...
                            <div class="mb-4">
                                ...
                                <select id="id_session" name="id_session"
                                    ...
                                    @foreach($sessions as $session)
                                        <option value="{{ $session->id }}" {{ $session->id == $filiere->id_session ? 'selected' : '' }}>
                                ...
                                <div class="mb-4">
                                    ...
                                    <div id="filiere_list" style="width: 100%; height: 300px; overflow-y: auto; border: 1px solid #d1d5db;">
                                        <div class="grid grid-cols-1 gap-4 p-2">
                                            @foreach($filieres as $filiereItem)
                                                <div class="flex items-center filiere-item">
                                                    <input type="checkbox" id="filiere_{{ $filiereItem->code_etape }}" name="filieres[]" value="{{ $filiereItem->code_etape }}"
                                                        class="mr-2 leading-tight" 
                                                        {{ in_array($filiereItem->code_etape, $distinctCodeEtapes) ? 'checked' : '' }}>
                                                    <label for="filiere_{{ $filiereItem->code_etape }}" class="text-gray-700">
                                                        {{ $filiereItem->version_etape }}
                                                    </label>
                                                </div>
                                            @endforeach
                                        </div>
